...
class CronTab(object):
    ...
    def get_cron_job(self,minute,hour,day,month,weekday,job,special):
        # change the split if hardcoded /etc/cron.d above changes
        sysv_style_cron_file = platform.system() in ['SunOS', 'HP-UX', 'AIX'] and self.cron_file is not None and self.cron_file.split('/')[3] == 'crontabs'
        if special:
            if self.cron_file and not sysv_style_cron_file:
